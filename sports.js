document.addEventListener("DOMContentLoaded", () => {
    ...
    function displayPlayers(players) {
        ...
        if (players) {
            players.forEach(player => {
                ...
                playerCard.classList.add('col-md-4');
                playerCard.innerHTML = `
                    <div class="card">
                        <img src="${player.strThumb}" class="img-fluid mb-3" alt="${player.strPlayer}" onerror="this.onerror=null;this.src='empty.png';">
                        <div class="card-body">
                            ...
                            <button class="btn btn-success hire-btn" data-id="${player.idPlayer}" ${isHired ? 'disabled' : ''}>${isHired ? 'Already Hired' : 'Hire'}</button>
                        </div>
                    </div>
                `;
    ...
    async function showPlayerDetails(playerId) {
        ...
        try {
            ...
            modalBody.innerHTML = `
                <img src="${player.strThumb}" class="img-fluid mb-3" alt="${player.strPlayer}" onerror="this.onerror=null;this.src='empty.png';">
                <p><strong>Player:</strong> ${player.strPlayer}</p>
    ...
    function hirePlayer(playerId, button) {
        if (playerCart.length >= maxPlayers) {
            alert("You can only hire up to 11 players.");
            return;
        }
